...
if($data_arr['method_name'] == "get_profile_details") { 
    ...
    if(empty($data_arr['std_id']) || $data_arr['std_id'] > 0){
        // PROFILE DETAIL
        $condition = array ( 
                         'select' 		=>	's.std_id, a.adm_fullname, a.adm_username, a.adm_phone, a.adm_email, s.std_dob, s.id_country, s.std_address_1, s.std_address_2, s.city_name, s.std_postal_address,  s.std_gender, c.country_name'
                        ,'join' 		=>	'INNER JOIN '.ADMINS.' a ON s.std_loginid = a.adm_id
                                             INNER JOIN '.COUNTRIES.' c ON s.id_country = c.country_id' 
                        ,'where' 		=>	array( 
                                                     'std_status'   => '1'
