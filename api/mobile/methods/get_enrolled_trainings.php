<?php
if ($data_arr['method_name'] == "get_enrolled_trainings") {
	...
	if(isset($data_arr['std_id']) && $data_arr['std_id'] != '') {
		...
		$ENROLLED_TRAININGS = $dblms->getRows(ENROLLED_COURSES.' ec', $conditions, $sql);
		if($ENROLLED_TRAININGS){
			foreach ($ENROLLED_TRAININGS as $course) :
				// Learn Type
                         $conditions = array ( 
                                            'select' 		=>	'id_type'
                                            ,'where' 		=>	array( 
						...
						$shortcourses[] = array(
							 'curs_id'           => $course['curs_id']
							,'curs_name'         => $course['curs_name']
							,'curs_photo'        => $photo
							,'curs_href'         => $course['curs_href']
							,'curs_rating'       => $curs_rating
			...
			endforeach;
			$rowjson['success'] = 1;
			$rowjson['MSG'] = 'Enrolled trainings fetched successfully.';
		}
		else{
			$rowjson['success'] = 0;
			$rowjson['MSG'] = 'No enrolled trainings found.';
		}
	} else {
		$rowjson['success'] = 0;
		$rowjson['MSG'] = 'Student ID is required.';
	}
	$rowjson['trainings_list'] = $shortcourses;
