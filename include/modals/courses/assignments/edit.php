            ...
            <div class="modal-body">
                    ...
                    <div class="col mb-2">
                        ...
                        <select class="form-control" data-choices name="id_week" required="">
                            ...
                            foreach(get_LessonWeeks() as $key => $val):
                                echo'<option value="'.$key.'" '.($key == $result['id_week'] ? 'selected' : '').'>'.get_CourseWise($_GET['curs_wise']).' '.$val.'</option>';
                            endforeach;
                            echo'
                        </select>
                    </div>
                ...
                <div class="row">
                    <div class="col mb-2">
                        ...
                        <input type="number" class="form-control" name="total_marks" value="'.$result['total_marks'].'" required>
                    </div>
                    <div class="col mb-2">
                        <label class="form-label">Passing Marks <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" required name="passing_marks" value="'.$result['passing_marks'].'">
                    </div>
                </div>
                <div class="row">
                    <div class="col mb-2">
                        ...
                        <select class="form-control" data-choices name="is_midterm">
                            ...
                            foreach(get_is_publish() as $key => $status):
                                echo'<option value="'.$key.'" '.($result['is_midterm'] == $key ? 'selected' : '').'>'.$status.'</option>';
                            endforeach;
                            echo '
                        </select>
                    </div>
                ...
                <div class="row">
                    <div class="col mb-2">
                        ...
                        <textarea name="detail" class="form-control" rows="3" required>'.$result['detail'].'</textarea>
                    </div>
                </div>
            </div>
