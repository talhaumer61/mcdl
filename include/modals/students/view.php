...
$condition = array(
                     'select'       =>  's.std_id, s.std_name, s.std_status, s.std_gender, s.city_name, s.std_address_1, s.std_address_2, s.id_skills, s.id_intrests, s.std_about, s.std_dob, a.adm_phone, a.adm_photo, a.adm_email, a.adm_username, GROUP_CONCAT(sk.skill_name) as std_intrests'
                    ,'join'         =>  'INNER JOIN '.ADMINS.' a ON a.adm_id = s.std_loginid
                                         INNER JOIN '.SKILLS.' sk ON FIND_IN_SET(sk.skill_id, s.id_intrests)'
                    ,'where'        =>  array(
                                                 's.is_deleted'     => 0
...
$condition = array (
                         'select'       =>	'ec.secs_id, ec.id_type, ec.secs_status, ec.id_curs, ec.id_mas, ec.id_ad_prg ,c.curs_name ,m.mas_name ,p.prg_name'
...
<div class="offcanvas-header border-bottom">
    <h5 class="offcanvas-title" id="offcanvasRightLabel">Student Details</h5>
