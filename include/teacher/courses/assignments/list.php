...
if (!empty($_GET['search_word'])) {
    ...
    $search_query   .= 'AND (caption LIKE "%'.$search_word.'%" OR date_start LIKE "%'.$search_word.'%" OR date_end LIKE "%'.$search_word.'%")';
...
$condition = array(
                    ...
                    ,'order_by'     =>  'id_week ASC'
                    ,'return_type'  =>  'count'
);
$count = $dblms->getRows(COURSES_ASSIGNMENTS, $condition);
echo'
<style type="text/css">
        .table-responsive {
...
$condition['order_by']      = "id_week ASC LIMIT " . ($page - 1) * $Limit . ",$Limit";
$condition['return_type']   = 'all';

$rowslist = $dblms->getRows(COURSES_ASSIGNMENTS, $condition);
if ($rowslist) {
    echo'
        ...
        <table class="table table-bordered table-nowrap mb-0">
            <thead class="table-light">
                <tr>
                    ...
                    <th width="70" class="text-center">Passing</th>
                        ...
                        <td class="text-center">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end" style="cursor: pointer;">';
                                    if($row['fileattach']){
                                        echo'<li><a href="uploads/files/'.LMS_VIEW.'/'.$row['fileattach'].'" class="dropdown-item" target="_blank"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View File</a></li>';
                                        echo'<li><a href="uploads/files/'.LMS_VIEW.'/'.$row['fileattach'].'" class="dropdown-item" download="'.$row['fileattach'].'"><i class="ri-download-cloud-2-fill align-bottom me-2 text-muted"></i> Download</a></li>';
                                    }
                                    echo'
                                    <li><a class="dropdown-item" onclick="showAjaxModalZoom(\'include/modals/'.moduleName().'/'.LMS_VIEW.'/edit.php?edit_id='.$row['id'].'&'.$redirection.'&curs_wise='.$curs['curs_wise'].'\');"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>
