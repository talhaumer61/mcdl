                ...
                <table class="table mb-0">
                    ...
                    <tbody>';
                        ...
                        foreach ($rowsList as $row) {                            
                            ...
                            $srno++;
                            // --- NEW: Print Certificate Redirection Logic ---
                            $print_certificate_btn = '';
                            $current_date = date('Y-m-d');

                            if ($row['status'] == 4 && !empty($row['leaving_date']) && strtotime($row['leaving_date']) < strtotime($current_date)) {
                                // Point directly to the standalone auto-download script
                                $print_certificate_btn = '<li><a class="dropdown-item" href="print_intern_certificate.php?intern_id='.$row['id'].'"><i class="ri-printer-fill align-bottom me-2 text-muted"></i> Print Certificate</a></li>';
                            }
                            echo '
                            <tr style="vertical-align: middle;">
