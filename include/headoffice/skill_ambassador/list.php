...
$condition  =   [ 
                    'select'        =>  'o.org_id, o.org_status, o.org_percentage, o.id_loginid, o.org_profit_percentage, a.adm_status, a.adm_photo, o.org_name, o.org_reg, o.org_phone, o.org_telephone, o.org_whatsapp, o.org_referral_link, o.org_link_from, o.org_link_to, o.org_type, o.allow_add_members, o.parent_org, o.cv_url, o.cv_file',
                ...
                <table class="table table-bordered mb-0">
                    <thead class="table-light">
                        <tr>                        
                            ...
                            <th width="120" class="text-center">Allow Members</th>
                    ...
                    <tbody>';
                        ...
                        foreach ($rowsList as $row) {                            
                            ...
                            <tr style="vertical-align: middle;">
                                ...
                                <td class="text-center">'.get_is_publish($row['allow_add_members']).'</td>
                                <td class="text-center">'.(($row['org_link_from'] != '0000-00-00' && date('Y-m-d', strtotime($row['org_link_from'])) <= date('Y-m-d') && date('Y-m-d', strtotime($row['org_link_to'])) >= date('Y-m-d')) ?'<span class="badge badge-gradient-success">Allowed</span>':'<span class="badge badge-gradient-danger">Not Allowed</span>').'</td>
                                <td class="text-center">'.get_leave($row['org_status']).'</td>
                                <td class="text-center">
                                    <div class="dropdown">
                                        <button class="btn btn-soft-primary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="ri-more-fill"></i></button>
                                        <ul class="dropdown-menu dropdown-menu-end" style="cursor: pointer;">                                        
                                            <li><a class="dropdown-item" href="dashboard.php?view=dashboard&name='.$row['org_name'].'&edit_id='.get_dataHashingOnlyExp($row['org_id'], true).'"><i class="ri-dashboard-fill align-bottom me-2 text-muted"></i> Dashboard</a></li>';
                                            if(!empty($row['cv_file'])){
                                                echo'<li><a class="dropdown-item" href="'.SITE_URL.'/uploads/files/organization_cv/'.$row['cv_file'].'" target="_blank"><i class="mdi mdi-eye align-bottom me-2 text-muted"></i> View Portfolio</a></li>';
                                            }
                                            echo'
                                            ...
                                            <li><a class="dropdown-item" onclick="showAjaxModalZoom(\'include/modals/'.$rootDir.'change_expiry_date/edit.php?org_id='.$row['org_id'].'\');"><i class="ri-map-pin-time-line align-bottom me-2 text-muted"></i> Change Expiry Date</a></li>';
                                            if($row['adm_status'] == 1){
                                                echo'<li><a class="dropdown-item" onclick="showAjaxModalZoom(\'include/modals/'.$rootDir.'change_password/edit.php?adm_id='.$row['id_loginid'].'\');"><i class="ri-lock-password-line align-bottom me-2 text-muted"></i> Change Password</a></li>';
                                            }
                                            echo'
                                            <li><a class="dropdown-item" href="?edit_id='.$row['org_id'].'"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>
                                            <li><a class="dropdown-item" onclick="confirm_modal(\''.moduleName().'.php?deleteid='.$row['org_id'].'&adm_id='.$row['id_loginid'].'\');"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>';
