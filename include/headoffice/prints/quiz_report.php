...
$condition      = array(
                 'select'       =>  'q.quiz_title, q.quiz_no_qns, q.quiz_totalmarks, q.quiz_passingmarks, c.curs_name, COUNT(qs.qzstd_id) AS std_attempted_count,  COUNT(CASE WHEN qs.qzstd_pass_fail = 1 THEN 1 END) AS std_pass_count, COUNT(CASE WHEN qs.qzstd_pass_fail = 0 THEN 1 END) AS std_fail_count'
                ,'join'         =>  'INNER JOIN '.QUIZ_STUDENTS.' AS qs ON qs.id_quiz = q.quiz_id
                                     ...
                                     INNER JOIN '.COURSES.' AS c ON c.curs_id = q.id_curs AND c.curs_status = 1 AND c.is_deleted = 0'
                ,'where'        =>  array(
                                        'q.quiz_status' => 1,
                                        'q.is_deleted'  => 0,
                                    )
