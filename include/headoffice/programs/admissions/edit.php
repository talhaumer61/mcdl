...
// ADMISSION_PROGRAMS
$condition = array ( 
                         'select' 	        => 'ap.id, ap.id_prg, ap.program, ap.intro_video, ap.status, ap.totalseats, ap.entrytest, ap.academic_sess, ap.morning, ap.evening, ap.weekend, ap.eligibility_criteria, ap.prg_for, ap.apply_enroll, ap.cohorts_deadlines, ap.class_profile, ap.programme_length, ap.career_outcomes, ap.alumni_benefits, ap.id_language, ap.payment_per_smester, ap.total_payments, ap.total_package, ap.examination_fee, ap.portal_fee, ap.library_fee, ap.student_card_fee, ap.library_mag_fee, ap.classdays, ap.deposit, ap.study_mode, ap.shortdetail, ap.detail, ap.metakeyword, ap.metadescription'
                        ,'where' 	        => array( 
                                                         'ap.is_deleted'    => '0'
                                                        ,'ap.id'            => cleanvars(LMS_EDIT_ID) 
                                                    )
                        ,'return_type' 	=> 'single' 
                    ); 
$row = $dblms->getRows(ADMISSION_PROGRAMS.' ap', $condition);
    ...
    <div class="col-lg-12 col-md-12">
        <div class="card">
            <div class="card-header bg-primary">
                <h5 class="mb-0 modal-title"><i class="ri-edit-circle-line align-bottom me-1"></i>Edit Admission Program</h5>
            </div>
            <form enctype="multipart/form-data" autocomplete="off" method="post" accept-charset="utf-8">
                        ...
                        <div class="card-body border">
                            <div class="row">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="form-control" data-choices name="program" required>
                                            ...
                                            foreach ($Programs as $key => $val):
                                                echo'<option value="'.$val['prg_id'].'|'.$val['prg_name'].'|'.$val['prg_eligibility'].'" '.(($row['id_prg'] == $val['prg_id'])? 'selected': '').'>'.$val['prg_name'].'</option>';
                                            endforeach;
                                            echo'
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="form-control" data-choices name="academic_sess" required>
                                            ...
                                            foreach ($ACADEMIC_SESSION as $key => $val):
                                                echo'<option value="'.$val['sess_id'].'" '.(($row['academic_sess'] == $val['sess_id'])? 'selected': '').'>'.$val['sess_name'].'</option>';
                                            endforeach;
                                            echo'
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="form-control" data-choices name="id_language" required>
                                            ...
                                            foreach ($languages as $lang):
                                                echo'<option value="'.$lang['lang_id'].'"  '.(($row['id_language'] == $lang['lang_id'])? 'selected': '').'>'.$lang['lang_name'].' - '.$lang['lang_code'].'</option>';
                                            endforeach;
                                            echo'
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="form-control" data-choices name="entrytest" required>
                                            ...
                                            foreach (get_is_publish() as $key => $val):
                                                echo'<option value="'.$key.'" '.(($row['entrytest'] == $key)? 'selected': '').'>'.$val.'</option>';
                                            endforeach;
                                            echo'
                                        </select>
                                    </div>
                                ...
                                foreach($inputFields as $name => $field): 
                                    ...
                                    <div class="'.$field['class'].'">
                                        <div class="mb-2">
                                            ...
                                            <input type="'.$field['type'].'" name="'.$name.'" id="'.$name.'" value="'.$row[$name].'" class="form-control" '.(($field['required'])? 'required': '').'>
                                        </div>
                                    </div>';
                                endforeach; 
                                echo '
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="form-control" data-choices name="study_mode" required>
                                            ...
                                            foreach (get_study_mode() as $key => $val):
                                                echo'<option value="'.$key.'" '.(($row['study_mode'] == $key)? 'selected': '').'>'.$val.'</option>';
                                            endforeach;
                                            echo'
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="intro_video" class="form-control" required="" value="'.$row['intro_video'].'">
                                </div> 
                                <div class="col-md-4">
                                    <label class="form-label mb-3">Session <span class="text-danger">*</span></label>
                                    <div class="row">
                                        <div class="col">
                                            <div class="mb-2">
                                                <div class="form-check mb-3">
                                                    <input class="form-check-input" type="checkbox" name="morning" id="morning" value="1" '.(($row['morning']==1)? 'checked': '').'>
                                        ...
                                        <div class="col">
                                            <div class="mb-2">
                                                <div class="form-check mb-3">
                                                    <input class="form-check-input" type="checkbox" name="evening" id="evening" value="1" '.(($row['evening']==1)? 'checked': '').'>
                                        ...
                                        <div class="col">
                                            <div class="mb-2">
                                                <div class="form-check mb-3">
                                                    <input class="form-check-input" type="checkbox" name="weekend" id="weekend" value="1" '.(($row['weekend']==1)? 'checked': '').'>
                        ...
                        <div class="card-body border">
                            <div class="row">
                                <div class="col">
                                    <div class="mb-2">
                                        ...
                                        <input class="form-control" name="shortdetail" required="" value="'.$row['shortdetail'].'"></input>
                                    </div>
                                </div>
                            </div>
                            <div class="row">';
                                foreach($textareaFields as $name => $field): 
                                    ...
                                    <div class="'.$field['class'].'">
                                        <div class="mb-2">
                                            ...
                                            <textarea class="form-control" id="'.$field['type'].'" name="'.$name.'" '.(($field['required'])? 'required': '').'>'.html_entity_decode($row[$name]).'</textarea>
                                        </div>
                                    </div>';
                                endforeach;
                                echo '
                            ...
                            <div class="row mb-2">
                                <div class="col">
                                    ...
                                    <input type="text" class="form-control" name="metakeyword" id="choices-text-remove-button" data-choices data-choices-limit="10" value="'.$row['metakeyword'].'"  data-choices-removeItem/>
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col">
                                    <label class="form-label">Description</label>
                                    <textarea class="form-control" name="metadescription">'.($row['metadescription']).'</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-footer">
                    <div class="hstack gap-2 justify-content-end">
                        ...
                        <button type="submit" class="btn btn-primary btn-sm" name="submit_edit"><i class="ri-edit-circle-line align-bottom me-1"></i>Edit '.moduleName(false).'</button>
                    </div>
                </div>
            </form>
        </div>
