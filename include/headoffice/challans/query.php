...
if(isset($_GET['deleteid'])) {
	...
	if($sqlDel) {		
		$sqlDel = $dblms->Update(CHALLAN_DETAIL, $values , "WHERE id_challan = '".cleanvars($latestID)."'");
		
		$condition = array(
							'select'       =>  'id_enroll'
							,'where'        =>  array(
														'challan_id'  => cleanvars($latestID)
													)
							,'return_type'  =>  'single'
		);
		$CHALLANS = $dblms->getRows(CHALLANS, $condition);
		if($CHALLANS){			
			$sqlDel = $dblms->Update(ENROLLED_COURSES, $values , " WHERE secs_id IN (".$CHALLANS['id_enroll'].") ");
		}
