...
// DEGREE
$condition      = [
                 'select'       =>  'd.deg_name, COUNT(DISTINCT ec.id_std) AS std_count'
                ,'join'         =>  'INNER JOIN '.ADMISSION_OFFERING.' AS ao ON ao.admoff_degree = d.deg_id
                                     LEFT JOIN '.ENROLLED_COURSES.' AS ec ON ec.id_ad_prg = d.deg_id'
                ,'where'        =>  [
                                        'd.deg_status'    => 1,
                                        'd.is_deleted'    => 0,
                                    ]
...
// DEGREE
$condition      = [
                 'select'       =>  'mt.mas_name, COUNT(DISTINCT ec.id_std) AS std_count'
...
// COURSES
$condition      = [
                 'select'       =>  'c.curs_name, COUNT(DISTINCT ec.id_std) AS std_count'
                ,'join'         =>  'INNER JOIN '.ADMISSION_OFFERING.' AS ao ON ao.admoff_degree = c.curs_id
                                     LEFT JOIN '.ENROLLED_COURSES.' AS ec ON ec.id_curs = c.curs_id'
                ,'where'        =>  [
                                        'c.curs_status'   => 1,
                                        'c.is_deleted'    => 0,
                                    ]
            ...
            <table class="table mb-0">
                <thead class="table-light">
                    <tr>                        
                        ...
                        <th class="text-center" width="200">Enrolled Students</th>
                    </tr>
                </thead>
                <tbody>';
                    $srno = 0;
                    foreach ($DEGREE        as $row) {
                        ...
                        <tr style="vertical-align: middle;">
                            ...
                            <td class="text-center">'.($row['std_count']?$row['std_count']:'No').'</td>
                        </tr>';
                    }
